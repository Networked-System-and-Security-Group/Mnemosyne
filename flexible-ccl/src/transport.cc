...
ncclResult_t ncclTransportP2pForcedConnect(struct ncclComm* comm, int channelId, int nrecv, int* peerRecv, int nsend, int* peerSend, int connIndex) {
  TRACE(NCCL_INIT, "nsend %d nrecv %d", nsend, nrecv);
  struct ncclChannel* channel = &comm->channels[channelId];
  uint64_t mask = 1UL << channel->id;
  for (int i=0; i<nrecv; i++) {
    ...
    channel->peers[peer]->recv[connIndex].connected = 0;
    free(channel->peers[peer]->recv[connIndex].transportResources);
    channel->peers[peer]->recv[connIndex].transportResources = NULL;
    comm->connectRecv[peer] |= mask;
  }
  for (int i=0; i<nsend; i++) {
    int peer = peerSend[i];
    if (peer == -1 || peer >= comm->nRanks || peer == comm->rank) continue;
    channel->peers[peer]->send[connIndex].connected = 0;
    free(channel->peers[peer]->send[connIndex].transportResources);
    channel->peers[peer]->send[connIndex].transportResources = NULL;
    comm->connectSend[peer] |= mask;
  }
  return ncclSuccess;
}
