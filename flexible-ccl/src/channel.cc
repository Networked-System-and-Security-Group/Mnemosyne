...
NCCL_PARAM(ReserveNum, "RESERVE_NUM", 2);
...
ncclResult_t initChannel(struct ncclComm* comm, int channelId) {
  ...
  int nPeers = nRanks + 1 /* Collnet */ + nvlsRanks /* NVLS */ + ncclParamReserveNum();
  ...
  if (channel->peers == NULL) {
    ...
    for (int r = 0; r < nRanks + ncclParamReserveNum(); r++) {
  ...
  channel->ring.userRanks = ncclMemoryStackAlloc<int>(&comm->memPermanent, nRanks + ncclParamReserveNum());
  NCCLCHECK(ncclCudaCallocAsync(&channel->devRingUserRanks, nRanks + ncclParamReserveNum(), sharedRes->deviceStream.cudaStream));
