...
static ncclResult_t ncclTopoGetTempXmlPath(struct ncclComm *comm, char *buf) {
  sprintf(buf, "./nvidia-topologyd/virtualTopology-%16lx.xml", comm->commHash);
  return ncclSuccess;
}
...
ncclResult_t ncclTopoGetSystem(struct ncclComm* comm, struct ncclTopoSystem** system) {
  ...
  struct ncclXml* xml;
  char* mem = NULL;
  const char defaultTopoLoc[] = "/var/run/nvidia-topologyd/virtualTopology.xml";
  char peersXmlPath[64];
  int* localRanks = NULL;
  int netDevCount = 0;
  ...
  if (xmlTopoFile) {
    ...
    NCCLCHECKGOTO(ncclTopoGetXmlFromFile(defaultTopoLoc, xml, 0), ret, fail);
  }
  if (xml->maxIndex == 0) {
    // Create top tag
  ...
  if (!xmlTopoFile) {
    xmlTopoFile = defaultTopoLoc;
  }
  NCCLCHECKGOTO(ncclTopoGetTempXmlPath(comm, peersXmlPath), ret, fail);
  if (xmlTopoFile && comm->rank == ncclParamTopoDumpFileRank()) {
    ...
    NCCLCHECKGOTO(ncclTopoDumpXmlToFile(peersXmlPath, xml), ret, fail);
  }
...
ncclResult_t ncclTopoGetSystemForNew(struct ncclComm* comm, struct ncclTopoSystem** system) {
  ncclResult_t ret = ncclSuccess;
  const char defaultTopoLoc[] = "/var/run/nvidia-topologyd/virtualTopology.xml";
  char peersXmlPath[64];
  struct ncclXml *xml;
  ...
  // int localRank = -1, nLocalRanks = 0;
  NCCLCHECK(xmlAlloc(&xml, NCCL_TOPO_XML_MAX_NODES));
  const char *xmlTopoFile = ncclGetEnv("NCCL_TOPO_DUMP_FILE");
  NCCLCHECKGOTO(ncclTopoGetTempXmlPath(comm, peersXmlPath), ret, fail);
  if (!xmlTopoFile) {
    xmlTopoFile = defaultTopoLoc;
  }
  NCCLCHECKGOTO(ncclTopoGetXmlFromFile(peersXmlPath, xml, 1), ret, fail);
  if (xml->maxIndex == 0) {
    // Create top tag
    struct ncclXmlNode* top;
    NCCLCHECKGOTO(xmlAddNode(xml, NULL, "system", &top), ret, fail);
  ...
  // Since net devices have already been detected when initializing, there's no need to detect again.
  // Since we create the topo based on prevoius xml, we need no branch removal & fusion.
