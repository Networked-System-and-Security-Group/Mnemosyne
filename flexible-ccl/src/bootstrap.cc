...
union ncclSocketAddress bootstrapNetIfAddr;
...
ncclResult_t bootstrapInitNew(ncclComm_t comm, const bootstrapState* peerState) {
  int rank = comm->rank;
  int nranks = comm->nRanks;
  struct bootstrapState *state;
  struct ncclSocket *proxySocket;
  ...
  // in case of failure, those resources will be free'd when calling bootstrapDestroy, so we can return immediatly
  NCCLCHECK(ncclCalloc(&state->peerProxyAddresses, nranks));
  memcpy(state->peerProxyAddresses, peerState->peerProxyAddresses, (nranks - 1) * sizeof(union ncclSocketAddress));
  NCCLCHECK(ncclCalloc(&proxySocket, 1));
  NCCLCHECK(createListenSocket(comm, comm->magic, proxySocket, state->peerProxyAddresses + rank, ncclSocketTypeProxy));

  NCCLCHECK(ncclCalloc(&state->peerProxyAddressesUDS, nranks));
  memcpy(state->peerProxyAddressesUDS, peerState->peerProxyAddressesUDS, (nranks - 1) * sizeof(uint64_t));
  NCCLCHECK(getUDS(state->peerProxyAddressesUDS + rank));

  // create a socket for others to reach out (P2P)
  union ncclSocketAddress peerSocketAddress;
  NCCLCHECK(createListenSocket(comm, comm->magic, &STATE_LISTEN(state, peerSocket), &peerSocketAddress, ncclSocketTypeBootstrap));
  NCCLCHECK(ncclCalloc(&state->peerP2pAddresses, nranks));
  memcpy(state->peerP2pAddresses, peerState->peerP2pAddresses, (nranks - 1) * sizeof(union ncclSocketAddress));
  memcpy(state->peerP2pAddresses + rank, &peerSocketAddress, sizeof(union ncclSocketAddress));
...
// For async bootstrap all-gather & bcast
struct ncclBootstrapAsyncJob {
...
ncclResult_t ncclBootstrapBroadcastFunc(struct ncclAsyncJob *job_) {
  ncclResult_t res = ncclSuccess;
  struct ncclBootstrapAsyncJob *job = (struct ncclBootstrapAsyncJob*)job_;
  struct bootstrapState *commState = (struct bootstrapState *)job->commState;
  int rank = commState->rank;
  int nranks = commState->nranks;
  int root = job->root;
  void *bcastData = job->data;
  int size = job->size;

  NCCLCHECKGOTO(bootstrapBroadcast(commState, rank, nranks, root, bcastData, size), res, fail);
...
NCCL_API(ncclResult_t, ncclBootstrapBroadcast, ncclComm_t comm, int root, void* buffer, size_t size);
ncclResult_t ncclBootstrapBroadcast(ncclComm_t comm, int root, void* buffer, size_t size) {
  ncclResult_t res = ncclSuccess;
  struct ncclBootstrapAsyncJob *job;
  NCCLCHECKGOTO(ncclCalloc(&job, 1), res, fail);
  job->commState = comm->bootstrap;
  job->root = root;
  job->data = buffer;
  job->size = size;
  NCCLCHECKGOTO(ncclAsyncLaunch(&job->base, ncclBootstrapBroadcastFunc, NULL, free, comm), res, fail);
